...
class Probabilistic_PR:
    ...
    def execute(self):
        if not self.convert_to_integers:
            cmd_string = './pr/optimal/pr2plan -d %s -i %s -o %s -P' % (self.domain, self.problem, self.obs_stream)
        else:
            cmd_string = './pr/optimal/pr2plan -d %s -i %s -o %s -P -Z %s' % (
            self.domain, self.problem, self.obs_stream, self.factor)
